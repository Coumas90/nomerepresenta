...
export const SeriesSection = ({
    ...
    artworks.forEach(artwork => {
      ...
      const observer = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting && !trackedArtworksRef.current.has(artwork.id)) {
          ...
          trackArtworkView(artwork.id, series.id, {
            hovered: false
          });
        }
      }, {
  ...
  return <div ref={sectionRef} className="mb-16" id={`series-${series.id}`}>
      {/* Sentinel element for intersection detection */}
      <div ref={sentinelRef} className="h-px" />
      
      {/* Sticky Header */}
      <div className={`sticky top-16 sm:top-20 bg-transparent z-40 pt-3 sm:pt-4 pb-4 sm:pb-6 transition-all duration-500 ${isVisible ? 'opacity-100 blur-0' : 'opacity-0 blur-sm'}`}>
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between">
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight lg:text-2xl">
              WORKS <span className="mx-1 sm:mx-2">&gt;</span> {series.name.toUpperCase()}
            </h1>
            
            {/* Only show button if series has description */}
            {series.description && <button onClick={onToggleDescription} className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 transition-opacity duration-300 hover:opacity-70" aria-label={isDescriptionExpanded ? "Collapse description" : "Expand description"}>
                {isDescriptionExpanded ? <Minus className="w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-300" /> : <Plus className="w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-300" />}
              </button>}
          </div>
        </div>
      </div>
      ...
      {series.description && <Collapsible open={isDescriptionExpanded}>
          <CollapsibleContent>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12">
        {artworks.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {artworks.map(artwork => <div key={artwork.id} data-artwork-id={artwork.id} className="group cursor-pointer animate-fade-in" onClick={() => onArtworkClick(artwork.id)} onMouseEnter={() => {
              ...
              hovered: true
                ...
                <div className="aspect-square bg-muted overflow-hidden mb-3 sm:mb-4 relative rounded-sm">
                  ...
                  <div className="absolute inset-0 transition-opacity duration-700 group-hover:opacity-0">
                    <ProgressiveImage src={artwork.image_url} alt={artwork.title} className="w-full h-full" />
                  </div>
                  ...
                  <div className="absolute inset-0 opacity-0 transition-opacity duration-700 group-hover:opacity-100 hidden sm:block">
                    <ProgressiveImage src={artwork.image_detail_url} alt={`${artwork.title} - Detail`} className="w-full h-full" eager skipInternalFade />
                  </div>
                </div>
                ...
                <div className="space-y-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
                  <h3 className="text-sm sm:text-base tracking-wide font-normal">
                    {artwork.title}
                  </h3>
