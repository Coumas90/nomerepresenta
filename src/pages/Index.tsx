            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {artworks.map(artwork => <div key={artwork.id} className="group cursor-pointer" onClick={() => navigate(`/artwork/${artwork.id}`)}>
                  <div className="aspect-square bg-muted overflow-hidden mb-4 relative">
                    ...
                    <img src={artwork.images[1] || artwork.image} alt={`${artwork.title} - Detail`} className="w-full h-full object-cover absolute inset-0 opacity-0 transition-opacity duration-700 group-hover:opacity-100" loading="lazy" />
                  </div>
